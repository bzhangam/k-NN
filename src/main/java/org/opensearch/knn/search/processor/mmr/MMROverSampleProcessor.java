...
public class MMROverSampleProcessor implements SearchRequestProcessor, SystemGeneratedProcessor {
    ...
    private static final int DEFAULT_OVERSAMPLE_SCALE = 10;
    ...
    private void preserveAndEnableFullSource(SearchRequest request, MMRRerankContext mmrContext) {
        ...
        // Nothing to preserve if fetching all fields is already enabled or source context is null
        if (currentSourceContext == null
            || (currentSourceContext.fetchSource()
                && currentSourceContext.includes().length == 0
                && currentSourceContext.excludes().length == 0)) {
            return;
        }

        // Preserve the original fetch source context for later restoration
        mmrContext.setOriginalFetchSourceContext(currentSourceContext);

        // Temporarily enable fetching all fields during MMR processing
